"""
South Carolina Procurement scraper

Source: https://procurement.sc.gov/solicitations/current
"""
import time, random
...
class SouthCarolinaProcurementScraper(BaseScraper):
    SEARCH_URL = "https://procurement.sc.gov/solicitations/current"
    def __init__(self):
        super().__init__("SC Procurement")
    def scrape(self):
        ...
        try:
            ...
            logger.warning(f"Error scraping SC: {e}")
        self.log_summary()
    ...
    def parse_opportunity(self, row):
        ...
        try:
            ...
            if not url.startswith('http'): url = f"https://procurement.sc.gov{url}"
            return {
                'title': title, 'organization': 'State of South Carolina',
                'description': None, 'eligibility': None, 'funding_amount': None,
                'deadline': None, 'category': categorize_opportunity(title, ''),
                'location': 'South Carolina', 'source': self.source_name,
                'source_url': url, 'opportunity_number': None, 'posted_date': None,
                'document_urls': [], 'opportunity_type': 'bid',
            }
        except: return None

def get_south_carolina_scrapers():
    return [SouthCarolinaProcurementScraper()]
